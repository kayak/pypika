...
class QuoteTests(unittest.TestCase):
    ...
    def test_use_double_quotes_on_alias_but_not_on_terms(self):
        ...
        self.assertEqual('SELECT a "bar" '
                         'FROM abc', q.get_sql(with_namespace=True))
    ...
    def test_use_double_quotes_on_alias_but_not_on_terms_with_joins(self):
        ...
        self.assertEqual('SELECT foo.a,bar.b '
                         'FROM abc "foo" '
                         'JOIN efg "bar" '
                         'ON foo.fk=bar.id', q.get_sql(with_namespace=True))
    ...
    def test_use_double_quotes_on_alias_but_not_on_terms(self):
        ...
        self.assertEqual('SELECT index "idx",SUM(value) "val" '
                         'FROM abc '
                         'GROUP BY "idx" '
                         'ORDER BY "idx"', q.get_sql(with_namespace=True))
