...
class CreateTableTests(unittest.TestCase):
    ...
    def test_create_table_with_select(self):
        select = Query \
            .from_(self.existing_table) \
            .select(self.existing_table.foo, self.existing_table.bar)
        ...
        with self.subTest('with temporary keyword'):
            ...
            self.assertEqual('CREATE TEMPORARY TABLE "abc" AS (SELECT "foo","bar" FROM "efg")', str(q))
