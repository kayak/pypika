...
class AggregationTests(unittest.TestCase):
    def test__count(self):
        q = Q.from_('abc').select(fn.Count('foo'))
    ...
    def test__avg(self):
        q = Q.from_('abc').select(fn.Avg('foo'))
...
class CastTests(unittest.TestCase):
    ...
    def test__cast__as(self):
        q = Q.from_(self.t).select(fn.Cast(self.t.foo, 'UNSIGNED'))
    ...
    def test__cast__unsigned(self):
        ...
        q2 = Q.from_(self.t).select(fn.Cast(self.t.foo, 'UNSIGNED'))
    ...
    def test__convert__utf8(self):
        q = Q.from_(self.t).select(fn.Convert(self.t.foo, 'utf8'))
